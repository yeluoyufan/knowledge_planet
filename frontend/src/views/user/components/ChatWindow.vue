<template>
        ...
        <div class="conversation-list" v-loading="loadingList">
          ...
          >
            ...
            <div class="info-wrapper">
              ...
              <div class="latest-msg">{{ getPreviewText(chat.latestMessage) }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="chat-main">
        <template v-if="currentChatUser">
          <div class="chat-header">
            <div class="header-user" @click="goToUserProfile(currentChatUser.userId)">
              <el-avatar :src="currentChatUser.avatar || defaultAvatar" :size="32" />
          ...
          <div class="message-container" ref="messageBoxRef">
            <div 
              ...
              :key="msg.id" 
              class="message-row"
              :class="{ 'row-self': isSelfMsg(msg) }"
            >
              <el-avatar 
                v-if="!isSelfMsg(msg)" 
                :src="msg.fromAvatar || defaultAvatar" 
                class="msg-avatar" :size="36" 
                @click.stop="goToUserProfile(msg.fromId)"
              />
              
              <div class="msg-content-wrapper">
                <div class="msg-bubble" :class="{ 'msg-bubble-auto': isAutoFollowMsg(msg) }">
                  <template v-if="isAutoFollowMsg(msg)">
                    <div class="auto-msg-tag">自动消息</div>
                    <div>{{ getPlainChatContent(msg.content) }}</div>
                  </template>
                  <template v-else>
                    {{ msg.content }}
                  </template>
  ...
  import { getConversationList, getMessageHistory, sendMessage, batchMarkRead, type ConversationVO, type MessageVO } from '@/api/message'
import { getUserInfo } from '@/api/user' // ✅ 引入获取用户信息接口
  
  const userStore = useUserStore()
  const route = useRoute()
  const router = useRouter()
  const defaultAvatar = 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'
  const AUTO_FOLLOW_PREFIX = '[AUTO_FOLLOW_THANKS]'
  const isSelfMsg = (msg: MessageVO) => String(msg.fromId) === String(userStore.userInfo?.id)
  const isAutoFollowMsg = (msg: MessageVO) => (msg.content || '').startsWith(AUTO_FOLLOW_PREFIX)
  const getPlainChatContent = (content: string) => (content || '').replace(AUTO_FOLLOW_PREFIX, '')
  const getPreviewText = (content: string) => {
    if ((content || '').startsWith(AUTO_FOLLOW_PREFIX)) {
      return `自动消息：${getPlainChatContent(content)}`
    }
    return content
  }
  const goToUserProfile = (userId: number | string | undefined) => {
    if (!userId) return
    ...
    try {
      ...
      if (!isBackground && route.query.chatUserId && !currentChatUser.value) {
        ...
        if (targetChat) {
          ...
          // 如果列表中没有这个人（从未聊过天），调用 getUserInfo 拿头像昵称构造一个临时会话对象
          try {
            const userRes: any = await getUserInfo(targetId)
            if (userRes.code === 0 || userRes.code === 200) {
              const u = userRes.data
              const tempChat: ConversationVO = {
                userId: u.id,
  ...
  .chat-window {
    ...
    .chat-main {
      ...
      .chat-header { height: 60px; border-bottom: 1px solid #eee; display: flex; align-items: center; padding: 0 20px; 
        .header-user { display: flex; align-items: center; gap: 10px; cursor: pointer; user-select: none; }
      ...
      .message-container {
        ...
        .message-row {
          ...
          .msg-content-wrapper {
            ...
            .msg-bubble-auto { background: linear-gradient(180deg, #f8fbff 0%, #eef6ff 100%); border: 1px solid #d9ecff; color: #1f2937; }
            .auto-msg-tag { display: inline-block; margin-bottom: 6px; padding: 2px 8px; border-radius: 999px; background: #409eff; color: #fff; font-size: 12px; line-height: 20px; }
