...
import com.hd.forum.mapper.MessageMapper;
 ...
 *
 * 设计说明：
 * - 私信、评论提醒、系统通知统一存储在 Message 表中，通过 type 字段区分
 * - 已读状态通过 isRead 维护，支持按类型统计未读数
 */
@Service
@RequiredArgsConstructor
public class MessageServiceImpl extends ServiceImpl<MessageMapper, Message> implements IMessageService {
    private static final int MESSAGE_CONTENT_MAX_LENGTH = 255;
    private static final Set<String> ALLOWED_MESSAGE_TYPES = Set.of("CHAT", "COMMENT", "SYSTEM", "LIKE", "FAVORITE", "AUDIT", "TOP");
    ...
    public void sendPrivateMessage(Long toUserId, String content) {
        ...
        createSystemNotification(fromUserId, toUserId, "CHAT", normalizedContent);
    }
    ...
    public void createSystemNotification(Long fromId, Long toId, String type, String content) {
        ...
        if (fromId == null || fromId < 0) {
            throw new BusinessException("发送者信息不合法");
        }
        if (fromId > 0 && userMapper.selectById(fromId) == null) {
            throw new BusinessException("发送者不存在");
        }
        if (userMapper.selectById(toId) == null) {
            throw new BusinessException("接收者不存在");
        }
        ...
        message.setType(type.trim().toUpperCase(Locale.ROOT));
    ...
    public Page<MessageVO> getMyMessages(Page<Message> page, String type) {
        ...
        if (normalizedType != null) {
            if ("SYSTEM".equals(normalizedType)) {
                // 如果是请求系统通知，则合并返回 SYSTEM, TOP, AUDIT 类型
                wrapper.in(Message::getType, Arrays.asList("SYSTEM", "TOP", "AUDIT"));
            } else {
                wrapper.eq(Message::getType, normalizedType);
            }
        }
    ...
    public List<MessageVO> getChatHistory(Long partnerId) {
        ...
        if (partnerId.equals(currentUserId)) {
            throw new BusinessException("不能查看与自己的聊天记录");
        }
    ...
    public void markAllAsRead() {
        ...
        wrapper.set(Message::getIsRead, true)
                ...
                .in(Message::getType, Arrays.asList("COMMENT", "SYSTEM", "LIKE", "FAVORITE", "AUDIT", "TOP"));
    ...
    public UnreadCountVO getUnreadCountByType() {
        ...
        return UnreadCountVO.builder()
                .total(chatCount + commentCount + systemCount + likeCount + favoriteCount + topCount + auditCount)
                .chat(chatCount)
    ...
    private String normalizeRequestedType(String type) {
        ...
        validateMessageType(normalizedType);
        return normalizedType;
    }

    private void validateMessageType(String type) {
        if (type == null || type.isBlank()) {
    ...
    private String normalizeContent(String content) {
        ...
        if (normalizedContent.length() > MESSAGE_CONTENT_MAX_LENGTH) {
            throw new BusinessException("消息内容不能超过255个字符");
        }
